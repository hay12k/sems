...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM students_table WHERE ClassId= :id order by StudentName");
...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT subject_table.SubjectName,subject_table.id FROM subject_by_class_table join  subject_table on  subject_table.id=subject_by_class_table.SubjectId WHERE subject_by_class_table.ClassId=:cid and subject_by_class_table.status!=:stts order by subject_table.SubjectName");
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM result_table WHERE StudentId=:id1 and ClassId=:id ");
